// open-chrome.js (supports normal Chrome and Tor modes)
...
(async () => {
  ...
  const profileDir = path.join(os.tmpdir(), 'puppeteer-profile-' + Date.now());
  ...
  if (MODE === 'chrome-tor') {
    ...
    args.unshift(`--proxy-server=socks5://127.0.0.1:${TOR_PROXY_PORT}`);
    console.log('Launching Chrome with Tor proxy...');
  } else {
    console.log('Launching normal Chrome...');
  }
