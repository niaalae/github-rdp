...
# Run direct.js with virtual display in an infinite loop
echo -e "${GREEN}Starting direct.js in INFINITE MODE...${NC}"
echo -e "${YELLOW}============================================${NC}"

while true; do
    echo -e "${BLUE}[$(date '+%Y-%m-%d %H:%M:%S')] Launching fresh instance...${NC}"
    
    xvfb-run -a -s "-screen 0 1920x1080x24" node "$SCRIPT_DIR/proton.js" "$@" 2>&1 | tee -a "$LOG_FILE"
