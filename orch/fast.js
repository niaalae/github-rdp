...
async function randomNoise(page) {
    try {
        ...
        // 1. Mouse movements (more steps, more variation)
        for (let i = 0; i < 15 + Math.floor(Math.random() * 10); i++) {
            ...
            // Occasional pause
            if (Math.random() > 0.8) await sleep(200 + Math.random() * 800);

            // Occasional wiggle
            if (Math.random() > 0.9) {
                for (let j = 0; j < 3; j++) {
                    await page.mouse.move(x + (Math.random() - 0.5) * 10, y + (Math.random() - 0.5) * 10, { steps: 5 });
                }
            }
        }

        // 2. Random scrolling (more frequent, varied amounts)
        for (let i = 0; i < 8 + Math.floor(Math.random() * 7); i++) {
            const scrollAmount = (Math.random() - 0.5) * 1500;
        ...
        // 4. Random hover over elements
        if (Math.random() > 0.5) {
            const elements = await page.$$('a, button, span, div');
...
async function generateGithubToken(page, creds) {
    ...
    try {
        ...
        await page.evaluate(() => {
            ...
            // Focus on the last checkbox to enable the "Enter" trick
            if (checkboxes.length > 0) {
                checkboxes[checkboxes.length - 1].focus();
                checkboxes[checkboxes.length - 1].style.outline = "5px solid blue";
            }
        });
        await sleep(2000);

        // 5. Submit
        console.log('Attempting to submit via "Enter" trick and "Generate token" button...');
        ...
        while (clickCount < 15) {
            ...
            // Try the "Enter" trick first on every loop
            console.log('Pressing Enter on focused element...');
            await page.keyboard.press('Enter');
            await sleep(2000);

            // Check if token appeared after Enter
            ...
            if (hasToken) {
                console.log('Token detected after Enter/Click. Proceeding...');
                break;
            }

            // 2. Try to find and click the button as well
            let genBtn = await page.evaluateHandle((selector) => {
                const buttons = Array.from(document.querySelectorAll(selector));
                // Try to find ANY button that looks like the generate button, even if visibility check is tricky
                let btn = buttons.find(el => {
                    const style = window.getComputedStyle(el);
                    const rect = el.getBoundingClientRect();
                    return style && style.display !== 'none' && rect.width > 0;
                });

                if (!btn) {
                    btn = Array.from(document.querySelectorAll('button')).find(el => {
                        const text = el.innerText.toLowerCase();
                        return text.includes('generate token') && !el.disabled;
                    });
                }
                return btn;
            }, genBtnSelector).catch(() => null);

            if (genBtn) genBtn = genBtn.asElement();

            if (genBtn) {
                try {
                    console.log(`Clicking Generate token button (attempt ${clickCount + 1})...`);
                    ...
                    await page.evaluate(el => el.click(), genBtn).catch(() => { });
                    await humanClick(page, genBtn).catch(() => { });
